...
def _format_label(label: Optional[str], score) -> Optional[str]:
    if label is None:
...
def draw_boxes(
    ...
    try:
        ...
        for i, (x1, y1, x2, y2) in enumerate(items):
            ...
            if text:
                cv2.putText(out, text, (xi1, max(12, yi1 - 6)),
...
def draw_text(
    ...
    try:
        import cv2
        cv2.putText(out, str(text), (int(xy[0]), int(xy[1])),
...
def draw_detections(
    ...
    for box, label, score, col in zip(boxes, labels, scores, colors):
        out = draw_boxes(out, [box], labels=[label], scores=[score],
