...
(run 2026-09-01 on-device) and in code references into the
platform repo.
...
on-device, daemons live; results below the table and in
...
**PLAT-1..5 were implemented and verified 2026-09-01**
on-device (platform branch `feat/dsp-service-p0`): the probe
...
### PLAT-6 measurement (2026-09-01, on-device)
...
("fix(camera-daemon): close fd_publisher dispatch races"), verified
on-device with a raw-socket probe: 4/4 pass — immediate-release 300/300,
...
Device-verified (ALL PASS): resize/crop/multi-crop content vs
...
Measured on-device (SDK-2 probe): a 4K-NV12 `resize_hw` call costs
